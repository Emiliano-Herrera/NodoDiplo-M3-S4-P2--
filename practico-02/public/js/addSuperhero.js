// Este es un contador de cuantos inputs se crearon por cada array
let contadoresArray = {
    poderes: 0,
    aliados: 0,
    enemigos: 0
...
// Agregar items a arrays, crea un div con la estructura de un input más un boton de eliminar ese input
...
function quitarItem(tipo, id) {
    ...
    // Validacion para poderes, evitando eliminar el ultimo input, 
    // si o si tiene que tener uno
...
function validarForm() {
    ...
    // en estas constantes obtiene los valores
    ...
    // Valida el nombre del heroe que no este vacío
    if (!nombreSuper) {
    ...
    // valida el nombre real del heroe que no este vacío
    if (!nombreReal) {
    ...
    // Valida la edad que no este vacío
    if (!edad) {
    ...
    // Valida cada poder individualmente con el forEach
    poderes.forEach(poder => {
        const valor = poder.value.trim(); // trim(eliminamos los espacios en blanco)
...
document.getElementById('superheroeForm').addEventListener('submit', async (e) => {
    ...
    if(validarForm()){
        const superheroe = {
            // construimos el objeto superheroe
        ...
        // tomamos los datos del array dinamicos
        ...
        // Preparamos y enviamos la peticion
        // Deshabilitar botón
        ...
        try {
            // Envia le peticion http al endpoint "/api/crearSuperHeroe" 
            // y usa el metodo POST para crear un nuevo heroe 
